...
export default async function generateSummary(
  ...
  try {
    ...
    xmlReader.on('tag:test', (test: any) => {
      const statusNode = test.children.find(
    ...
    xmlReader.on('tag:statistics', (statistics: any) => {
      const stat = statistics.children
        .find((child: any) => child.name === 'total')
    ...
    const passPercentage = parseFloat(
      getPassPercentage(statistics.pass, statistics.fail)
    )
...
function getPassPercentage(pass: number, fail: number) {
  if (pass !== 0 && fail === 0) return (100).toFixed(2)
